...
func (m *MockQuerier) ListPaymentInstallmentsByPlanID(ctx context.Context, paymentPlanID uuid.UUID) ([]*PaymentInstallment, error) {
	m.ctrl.T.Helper()
	ret := m.ctrl.Call(m, "ListPaymentInstallmentsByPlanID", ctx, paymentPlanID)
	ret0, _ := ret[0].([]*PaymentInstallment)
	ret1, _ := ret[1].(error)
	return ret0, ret1
}
